...
def text_handler(message):
    ...
    if text == "💰 Balance":
        ...
        bot.send_message(
            ...
            f"_আপনার রেফারেল মাইনিং শেষ করলে আপনি রিওয়ার্ডের {ref_pct}% কমিশন পাবেন।_\n\n"
    ...
    # ADMIN MENU HANDLERS
        ...
        markup.add(InlineKeyboardButton(f"💸 Min Withdraw: {s['min_withdraw']} USDT", callback_data="set_min_withdraw"))
        markup.add(InlineKeyboardButton(f"⛏ Mining Reward: {s['mining_reward']} USDT", callback_data="set_mining_reward"))
        ...
        bot.send_message(
            ...
            f"_যে সেটিং পরিবর্তন করতে চান সেই বাটনে ক্লিক করুন।_",
            reply_markup=markup,
            parse_mode="Markdown"
        )
    ...
    # SETTINGS STEP HANDLERS
        ...
        try:
            if step == "set_channel":
                ...
                save_db(db)
                bot.send_message(chat_id, f"✅ *চ্যানেল আপডেট হয়েছে:* `{text}`", parse_mode="Markdown")
            elif step == "set_refs":
                val = int(text)
                if val < 0: raise ValueError
                db["bot_settings"]["req_refs"] = val
                save_db(db)
                bot.send_message(chat_id, f"✅ *রেফারেল সংখ্যা আপডেট:* *{val}*", parse_mode="Markdown")
            elif step == "set_min_withdraw":
                val = float(text)
                if val <= 0: raise ValueError
                db["bot_settings"]["min_withdraw"] = val
                save_db(db)
                bot.send_message(chat_id, f"✅ *নূন্যতম উইথড্র আপডেট:* *{val} USDT*", parse_mode="Markdown")
            elif step == "set_mining_reward":
                val = float(text)
                if val <= 0: raise ValueError
                db["bot_settings"]["mining_reward"] = val
                save_db(db)
                bot.send_message(chat_id, f"✅ *মাইনিং রিওয়ার্ড আপডেট:* *{val} USDT*", parse_mode="Markdown")
            elif step == "set_mining_hours":
                val = float(text)
                if val <= 0: raise ValueError
                db["bot_settings"]["mining_hours"] = val
                save_db(db)
                bot.send_message(chat_id, f"✅ *মাইনিং সময় আপডেট:* *{val} hour(s)*", parse_mode="Markdown")
            elif step == "set_ref_mining_percent":
                val = int(text)
                if val < 0 or val > 100: raise ValueError
                db["bot_settings"]["ref_mining_percent"] = val
                save_db(db)
                bot.send_message(chat_id, f"✅ *রেফারেল মাইনিং কমিশন আপডেট:* *{val}%*", parse_mode="Markdown")
        except (ValueError, TypeError):
            bot.send_message(chat_id, "❌ *সঠিক মান দিন।*", parse_mode="Markdown")
        user_steps.pop(user_id, None)
    ...
    if data == "check_join":
        ...
        if is_app:
            ...
            try:
                bot.send_message(
                    t_user,
                    f"❌ *Withdrawal Rejected!*\n\n*{amount} USDT* has been refunded to your balance.",
                    parse_mode="Markdown"
                )
            except:
                pass
        ...
        prompts = {
            "set_channel":           "📢 নতুন চ্যানেল লিঙ্ক পাঠান:\n_উদাহরণ: @MyChannel_",
            "set_refs":              "👥 রেফারেল সংখ্যা পাঠান:\n_উদাহরণ: 3_",
            "set_min_withdraw":      "💸 নূন্যতম উইথড্র পরিমাণ পাঠান:\n_উদাহরণ: 5_",
            "set_mining_reward":     "⛏ মাইনিং রিওয়ার্ড পাঠান (USDT):\n_উদাহরণ: 0.10_",
            "set_mining_hours":      "⏱ মাইনিং সময় পাঠান (ঘণ্টা):\n_উদাহরণ: 1_",
            "set_ref_mining_percent":"📈 রেফারেল মাইনিং কমিশন % পাঠান:\n_উদাহরণ: 10_",
        }
