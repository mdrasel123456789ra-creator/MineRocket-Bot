...
# Withdrawal approval group – all requests go here exclusively
WITHDRAWAL_GROUP_ID = -1004387474085
        ...
        if referrer and str(referrer) in db2["users"]:
            ...
            try:
                bot.send_message(
                    ...
                    f"💰 Mining Reward: {reward} MRT\n"
                    f"📈 Commission: {commission} MRT\n"
...
def start_command(message):
    ...
    bot.send_message(
        ...
        f"*Earn & Withdraw MRT Coin Easily*\n\n"
...
def text_handler(message):
    ...
    if text == "💰 Balance":
        ...
        bot.send_message(
            chat_id,
            f"💰 *ACCOUNT BALANCE*\n\n"
            f"Available: *{round(bal, 2)} MRT*\n"
            f"Min Withdraw: *{min_wd} MRT*",
            parse_mode="Markdown"
        )
        ...
        ref_pct    = db["bot_settings"].get("ref_mining_percent", 10)
        ref_count  = user_data["refs"]
        ref_link   = f"https://t.me/{bot_username}?start={user_id}"
        bot.send_message(
            chat_id,
            f"👥 *Affiliate Program*\n\n"
            f"📈 Mining Commission: *{ref_pct}%* per mining cycle\n"
            f"👤 Total Referrals: *{ref_count}*\n\n"
            f"_Every time your referral completes mining, you earn {ref_pct}% of their reward as commission._\n\n"
        ...
        for tid, task in active_tasks.items():
            ...
            if not done:
                ...
                status_text = f"Reward: *{reward} MRT*"
            else:
        ...
        if my_refs < req_refs:
            ...
            bot.send_message(
                ...
                f"Minimum Withdraw: *{min_wd} MRT*\n"
                f"Your Balance: *{round(bal,2)} MRT*",
                parse_mode="Markdown"
            )
        else:
            bot.send_message(
                chat_id,
                f"📤 *Enter Withdrawal Amount (MRT):*\n\n"
                f"Minimum: *{min_wd} MRT*\n"
                f"Available: *{round(bal,2)} MRT*",
                parse_mode="Markdown"
            )
    ...
    # ADMIN MENU HANDLERS
        ...
        s = db["bot_settings"]
        markup = InlineKeyboardMarkup()
        markup.add(InlineKeyboardButton(f"📢 Main Channel: {s['channel_link']}", callback_data="set_channel"))
        markup.add(InlineKeyboardButton(f"👥 Req Referrals: {s['req_refs']}", callback_data="set_refs"))
        markup.add(InlineKeyboardButton(f"💸 Min Withdraw: {s['min_withdraw']} MRT", callback_data="set_min_withdraw"))
        markup.add(InlineKeyboardButton(f"⛏ Mining Reward: {s['mining_reward']} MRT", callback_data="set_mining_reward"))
        markup.add(InlineKeyboardButton(f"⏱ Mining Hours: {s['mining_hours']}h", callback_data="set_mining_hours"))
        markup.add(InlineKeyboardButton(f"📈 Referral Mining %: {s.get('ref_mining_percent', 10)}%", callback_data="set_ref_mining_percent"))
        bot.send_message(
            chat_id,
            f"⚙️ *Bot Configuration*\n\n"
            f"📢 Main Channel: `{s['channel_link']}`\n"
            f"👥 Req Referrals: *{s['req_refs']}*\n"
            f"💸 Min Withdraw: *{s['min_withdraw']} MRT*\n"
            f"⛏ Mining Reward: *{s['mining_reward']} MRT*\n"
            f"⏱ Mining Duration: *{s['mining_hours']} hour(s)*\n"
            f"📈 Referral Mining %: *{s.get('ref_mining_percent', 10)}%*\n\n"
            f"_Tap a button to update that setting._",
            reply_markup=markup,
            parse_mode="Markdown"
        )
    ...
    # STEP HANDLERS (No logic changes)[cite: 1]
        ...
        try:
            ...
            if amount <= 0 or amount > bal:
                ...
                bot.send_message(chat_id, f"❌ *Minimum withdrawal is {min_wd} MRT.*", parse_mode="Markdown")
                user_steps.pop(user_id, None)
            else:
                db["users"][user_id]["withdraw_amount"] = amount
                save_db(db)
                bot.send_message(chat_id, "🏦 *Enter your BEP(20) Wallet Address:*", parse_mode="Markdown")
        ...
        try:
            bot.send_message(WITHDRAWAL_GROUP_ID, group_msg, reply_markup=markup)
    ...
    # TASK EDIT STEP HANDLERS (No logic changes)[cite: 1]
        ...
        if phase == "edit_title" and message.from_user.id == ADMIN_ID:
            ...
            try:
                ...
                save_db(db)
                bot.send_message(chat_id, f"✅ Reward updated: *{reward} MRT*", parse_mode="Markdown")
            except ValueError:
                bot.send_message(chat_id, "Invalid amount.", parse_mode="Markdown")
            user_steps.pop(user_id, None)

    # SETTINGS STEP HANDLERS
    elif step in ["set_channel", "set_refs", "set_min_withdraw", "set_mining_reward", "set_mining_hours", "set_ref_mining_percent"] and message.from_user.id == ADMIN_ID:
        try:
            if step == "set_channel":
                ...
                db["bot_settings"]["req_refs"] = val
                save_db(db)
                bot.send_message(chat_id, f"✅ *Required referrals updated:* *{val}*", parse_mode="Markdown")
            elif step == "set_min_withdraw":
                val = float(text)
                if val <= 0: raise ValueError
                db["bot_settings"]["min_withdraw"] = val
                save_db(db)
                bot.send_message(chat_id, f"✅ *Minimum withdrawal updated:* *{val} MRT*", parse_mode="Markdown")
            elif step == "set_mining_reward":
                val = float(text)
                if val <= 0: raise ValueError
                db["bot_settings"]["mining_reward"] = val
                save_db(db)
                bot.send_message(chat_id, f"✅ *Mining reward updated:* *{val} MRT*", parse_mode="Markdown")
            elif step == "set_mining_hours":
                val = float(text)
                if val <= 0: raise ValueError
                db["bot_settings"]["mining_hours"] = val
                save_db(db)
                bot.send_message(chat_id, f"✅ *Mining duration updated:* *{val} hour(s)*", parse_mode="Markdown")
            elif step == "set_ref_mining_percent":
                val = int(text)
                if val < 0 or val > 100: raise ValueError
                db["bot_settings"]["ref_mining_percent"] = val
                save_db(db)
                bot.send_message(chat_id, f"✅ *Referral mining commission updated:* *{val}%*", parse_mode="Markdown")
        except (ValueError, TypeError):
            bot.send_message(chat_id, "❌ *Invalid value. Please send a correct number.*", parse_mode="Markdown")
        user_steps.pop(user_id, None)
...
def callback_handler(call):
    ...
    if data == "check_join":
        ...
        try:
            ...
            if status in ['member', 'administrator', 'creator']:
                ...
                save_db(db)
                bot.send_message(call.message.chat.id, f"✅ *Task Completed!* Received *{reward} MRT*", parse_mode="Markdown")
                bot.answer_callback_query(call.id)
            else:
        ...
        is_app = data.startswith("wgrp_app_")
        # callback_data format: wgrp_app_{user_id}_{amount} or wgrp_rej_{user_id}_{amount}
        # split on _ with maxsplit=3 to safely handle user_ids and amounts
        parts = data.split("_", 3)
        t_user = parts[2]
        amount = float(parts[3])

        # Prevent duplicate approval/rejection: check message text
        current_text = call.message.text or ""
        if "✅ APPROVED" in current_text or "❌ REJECTED" in current_text:
            bot.answer_callback_query(call.id, "Already processed!", show_alert=True)
            return

        # Only group admins may approve or reject
        try:
            admin_status = bot.get_chat_member(WITHDRAWAL_GROUP_ID, call.from_user.id).status
            if admin_status not in ("administrator", "creator"):
                bot.answer_callback_query(call.id, "❌ Only group admins can do this!", show_alert=True)
                return
        except Exception as e:
            bot.answer_callback_query(call.id, "Could not verify admin status.", show_alert=True)
            return
        ...
        if is_app:
            ...
            try:
                bot.send_message(
                    t_user,
                    f"✅ *Withdrawal Approved!*\n\n*{amount} MRT* has been sent.",
                    parse_mode="Markdown"
                )
            except:
                pass
            # Update group message: keep original text, append status, disable buttons
            ...
            try:
                bot.send_message(
                    t_user,
                    f"❌ *Withdrawal Rejected!*\n\n*{amount} MRT* has been refunded to your balance.",
                    parse_mode="Markdown"
                )
            except:
                pass
    ...
    elif data in ["set_channel", "set_refs", "set_min_withdraw", "set_mining_reward", "set_mining_hours", "set_ref_mining_percent"]:
        prompts = {
            "set_channel":           "📢 Send the new channel link:\n_Example: @MyChannel_",
            "set_refs":              "👥 Send the required referral count:\n_Example: 3_",
            "set_min_withdraw":      "💸 Send the minimum withdrawal amount (MRT):\n_Example: 5_",
            "set_mining_reward":     "⛏ Send the mining reward amount (MRT):\n_Example: 0.10_",
            "set_mining_hours":      "⏱ Send the mining duration in hours:\n_Example: 1_",
            "set_ref_mining_percent":"📈 Send the referral mining commission (%):\n_Example: 10_",
        }
