...
def text_handler(message):
    ...
    if text == "💰 Balance":
        ...
        # FEATURE #2: Withdrawal Approval Group Implementation
        address = text
        ...
        try:
            ...
            print(f"Error sending to withdrawal group: {e}")
...
def callback_handler(call):
    ...
    if data == "check_join":
        ...
        # Check if the clicking user is a group admin
        chat_status = bot.get_chat_member(call.message.chat.id, call.from_user.id).status
        if chat_status not in ['administrator', 'creator']:
            bot.answer_callback_query(call.id, "❌ Only group admins can perform this action!", show_alert=True)
            return
        ...
        # Get current message text to preserve it while updating status
        current_text = call.message.text
        
        if is_app:
            try:
                bot.send_message(t_user, f"✅ *Withdrawal Approved!* *{amount} USDT* sent.", parse_mode="Markdown")
            except: pass
            new_text = current_text.replace("⏳ Pending", "✅ Approved")
            ...
            if t_user in db2["users"]:
                ...
                save_db(db2)
            try:
                bot.send_message(t_user, f"❌ *Withdrawal Rejected!* *{amount} USDT* returned.", parse_mode="Markdown")
            except: pass
            new_text = current_text.replace("⏳ Pending", "❌ Rejected")
            bot.edit_message_text(new_text, call.message.chat.id, call.message.message_id, reply_markup=None)
        bot.answer_callback_query(call.id)
