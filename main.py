...
# অ্যাডমিন আইডি Render থেকে নিবে, না পেলে ডিফল্ট হিসেবে আপনার আইডি (8428347588) ব্যবহার করবে
try:
...
def load_db():
    if not os.path.exists(DB_FILE):
        return {"users": {}, "bot_settings": {
            "channel_link": "@MineRocket", # ডিফল্ট চ্যানেল
...
db = load_db()
user_steps = {} # ইউজারের বর্তমান ইনপুট স্টেপ মনে রাখার জন্য
...
def admin_command(message):
    if message.from_user.id == ADMIN_ID:
        ...
        bot.send_message(message.chat.id, "👨‍💻 *Mine Rocket Admin Dashboard*\n\nWelcome to the control panel.", reply_markup=markup, parse_mode="Markdown")

# ==========================================
# ⏰ অটোমেটিক মাইনিং রিওয়ার্ড ফাংশন
# ==========================================
def give_mining_reward(user_id, chat_id):
    ...
    if uid in db["users"] and db["users"][uid]["mining_status"] == "running":
        ...
        save_db(db)
        try:
            bot.send_message(chat_id, "✅ *Mining Completed!*\n\nYou received *0.10 USDT*. You can click on ⛏ Mining to start again!", parse_mode="Markdown")
        except:
            pass

# ==========================================
# ⌨️ সাধারণ টেক্সট ও মেনু বাটন হ্যান্ডলার
# ==========================================
...
def text_handler(message):
    ...
    if text == "💰 Balance":
        ...
        if user_data["mining_status"] == "running":
            ...
            save_db(db)
            bot.send_message(chat_id, "⚡ *Mining Started!*\n\nYour mining will be completed in 1 hour. We will notify you automatically!", parse_mode="Markdown")
        ...
        if not task_channel:
            ...
            bot.send_message(chat_id, "📝 *New Task Available!*\n\nJoin the channel below to earn a reward.", reply_markup=markup, parse_mode="Markdown")

    elif text == "📤 Withdraw":
        req_refs = db["bot_settings"]["req_refs"]
        my_refs = user_data["refs"]
        if my_refs < req_refs:
            bot.send_message(chat_id, f"❌ *Withdrawal Locked!*\n\nYou need at least *{req_refs}* referrals. Your current: *{my_refs}*", parse_mode="Markdown")
        else:
    ...
    # --- Admin Menu ---
        ...
        bot.send_message(chat_id, "Send the channel link for the task (e.g. @MyTaskChannel):")
    ...
    # --- Step Inputs ---
        ...
        # অ্যাডমিন চ্যানেল বা অ্যাডমিন আইডিতে মেসেজ পাঠানো
        try:
    ...
    # --- Admin Steps ---
    elif step == "add_task" and message.from_user.id == ADMIN_ID:
...
def callback_handler(call):
    ...
    if data == "check_join":
        ...
        try:
            ...
            bot.answer_callback_query(call.id, "⚠️ Error! Ensure bot is admin in the channel.", show_alert=True)
        ...
        try:
            ...
            if status in ['member', 'administrator', 'creator']:
                ...
                db["users"][user_id]["balance"] += 0.50 # টাস্ক রিওয়ার্ড
                save_db(db)
                bot.send_message(call.message.chat.id, "✅ *Task Completed!* You received *0.50 USDT*.", parse_mode="Markdown")
            else:
                bot.answer_callback_query(call.id, "❌ You haven't joined the task channel yet!", show_alert=True)
        except:
            bot.answer_callback_query(call.id, "⚠️ Error checking status.", show_alert=True)
